...
export const Button = forwardRef((props: ButtonProps, ref: Ref<HTMLButtonElement>) => {
  const { title, children, className, size = "md", variant = "default", ...rest } = props
  ...
  return (
    <button
      ref={ref}
      className={`rounded border mx-1 ${sizeVariants[size]} ${buttonVariants[variant]} ${className ? className : ""}`}
      {...rest}
    >
      {title} {children}
    </button>
